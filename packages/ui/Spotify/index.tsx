...
export default function NowPlaying() {
  ...
  return (
    <>
      {data?.songUrl && (
        <div className="flex flex-col w-full">Now Playing:</div>
      )}
              <svg
          className="flex flex-col h-4 w-4 ml-auto mt-[-2px]"
          viewBox="0 0 168 168"
        >
          <path
            fill="#1ED760"
      ...
      <div className="flex flex-row-reverse items-center sm:flex-row mb-8 space-x-0 sm:space-x-2 w-full">
        <div className="flex-col sm:flex-row w-full max-w-full truncate">
          {data?.songUrl ? (
            <a
